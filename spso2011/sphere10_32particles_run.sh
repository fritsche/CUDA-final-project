...
gcc -Wall cpu_spso2011.c -o cpu_v0.1 -lm
cpu=./cpu_v0.1
echo "compiling for GPU"
nvcc -arch=sm_20 gpu_spso2011.cu -o gpu_v0.1
gpu=./gpu_v0.1
...
echo "" > cpu_spso2011.txt
echo "" > cpu_spso2011.time
i=1
for seed in ${seeds[@]}; do
	echo "CPU: $i"
	START=$(date +%s.%N)
	$cpu $seed >> cpu_spso2011.txt
	END=$(date +%s.%N)
	DIFF=$(echo "$END - $START" | bc)
	echo $DIFF >> cpu_spso2011.time
	((i++))
done

echo "" > gpu_spso2011.txt
echo "" > gpu_spso2011.time
i=1
for seed in ${seeds[@]}; do
	echo "GPU: $i"
	START=$(date +%s.%N)
	$gpu $seed >> gpu_spso2011.txt
	END=$(date +%s.%N)
	DIFF=$(echo "$END - $START" | bc)
	echo $DIFF >> gpu_spso2011.time
	((i++))
done
